...
async function getLeagueRanking() {
  ...
  const exactIndex = ranking.findIndex(
    name => name.toLowerCase() === LEAGUE_NAME.toLowerCase()
  );
...
function getPassedLeagues(oldRank, newRank, oldRanking, currentRanking) {
  if (!Array.isArray(oldRanking) || !Array.isArray(currentRanking)) {
    return [];
  }

  if (newRank < oldRank) {
    return oldRanking.slice(newRank - 1, oldRank - 1);
  }

  return currentRanking.slice(oldRank, newRank);
}
...
function formatHoursSince(unixSeconds) {
  ...
  const hours = Math.max(
    0,
    (Date.now() - Number(unixSeconds) * 1000) / 3600000
  );
...
function avatarUrl(userId) {
  return `https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=${encodeURIComponent(userId)}&size=150x150&format=Png&isCircular=true`;
}
...
async function resolveAvatar(userId) {
  const response = await fetchBuffer(avatarUrl(userId));
...
async function resolveLeagueIcon(icon) {
  const match = String(icon || '').match(/rbxassetid:\/\/(\d+)/i);
...
function getContributors(league) {
  ...
  const owner =
    league.Owner && league.Owner.UserID != null
      ? league.Owner
      : null;
  ...
  return members
    ...
    .map(member => {
      ...
      return {
        ...
        points: Number(contribution?.Points || 0),
        timestamp: Number(contribution?.Timestamp || 0),
      };
    })
    .sort((a, b) => b.points - a.points);
}

function makeSnapshot(league, contributors) {
  return {
    timestamp: Date.now(),
    points: Number(league.Points || 0),

    members: contributors.map(member => ({
      userId: member.userId,
      name: member.name,
...
function findHistorySnapshot(
  ...
  for (const snapshot of history) {
    const distance = Math.abs(
      Number(snapshot.timestamp) - targetMs
    );

    if (
      Number(snapshot.timestamp) <= targetMs + toleranceMs &&
...
async function checkRank() {
  ...
  // Keep one dashboard message and update it on every successful check.
